...
def bubble_sort(arr: list[int], asc: bool = True):
    n = len(arr)
    ...
    for i in range(0, n):
        ...
        for j in range(0, n - i - 1):
            ...
            # Check if jth element is smaller than (j+1)th element 
            if arr[j] < arr[j+1]:
                
                # if asc=True then we want to keep the elts as they are, otherwise we swap
                if not asc:
                    tmp = arr[j+1]
                    arr[j+1] = arr[j]
                    arr[j] = tmp
                    ...
                    swapped = True
            # Check if jth element is larger than (j+1)th element 
            elif arr[j] > arr[j+1]:

                # if asc=True then we want to swap the elements, otherwise we keep as they are
                if asc:
                    tmp = arr[j+1]
                    arr[j+1] = arr[j]
                    arr[j] = tmp
...
print(bubble_sort(test_array, True))
# Sort descending order
print(bubble_sort(test_array, False))

# Time for benchmarking and to show O(n^2)
time_taken = timeit.timeit(lambda: bubble_sort(test_array, True), number = 1000)
print(f"Average time over 1000 runs for a length 10 array: {time_taken/1000:.6f} seconds")
...
time_taken_medium = timeit.timeit(lambda: bubble_sort(medium_test_array, True), number = 1000)
time_taken_larger = timeit.timeit(lambda: bubble_sort(larger_test_array, True), number = 1000)
print(f"Average time over {n1} runs for a length 100 array: {time_taken_medium/1000:.6f} seconds")
print(f"Average time over {n2} runs for a length 1000 arrray: {time_taken_larger/1000:.6f} seconds")
